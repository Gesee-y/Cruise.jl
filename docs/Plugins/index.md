...
- `pmap!(f, plugin)` : It will topologically apply the function `f` on the node of the plugin. It will run nofe# Cruise 0.3.0 documentation: Plugins
...
@enum CRPluginStatus begin
    PLUGIN_OFF # The plugin is not initialized
    PLUGIN_DEPRECATED # The plugin result are deprecated and needs an update
    PLUGIN_OK # The plugin is inited
    PLUGIN_ERR # The plugin encountered an error
end
```
...
- `smap!(f, plugin)` : It will topologically and sequentially apply the function `f` on the node of the plugin
- `pmap!(f, plugin)` : It will topologically apply the function `f` on the node of the plugin. It will run nodes that are a the same level in parallel to ensure it's thread-safe.