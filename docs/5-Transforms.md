...
- `space` is the vector space in which the transformation accour. basically the vector space indicate along which axis the `position` field refers to.
- `position` is the emplacement of the object relatively to his space
- `angle` is the rotation arounf eeach axis
...
This matrix can safely be passed to to API like OpenGL
