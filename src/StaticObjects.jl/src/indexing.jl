...
"""
#=function Base.setindex!(array::AbstractSArray{T,D,N},val::T,i::Int) where{T <: Any ,D <: Tuple,N}
	L = length(array)
	...
	if i == 1
		data = append_tuple((val,), (AD[begin+1:end]))
	elseif i == L
		append_tuple(AD[begin:end-1], (val,))
	else
		append_tuple(AD[begin:i-1], (val,), (AD[i+1:end]))
	end
